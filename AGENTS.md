...
- 与廖送平/任意 Agent 的聊天**不再**由本机自动化同步为 Raw（2026-09-15 已关闭 agent-chat）；接手时仍以已落盘文档为准，不要求翻聊天原文。
...
- **Git**：完成一件可独立还原的改动后自动 commit，中文说明；不自动 push（明示才推）；细则见 [协作偏好](06-长期记忆/协作偏好.md) 与 Cursor Rule [git-自动细粒度提交.mdc](.cursor/rules/git-自动细粒度提交.mdc)。
...
- 采集桥：妙记默认仅登记 Raw README（不落本地源文件），与云文档口径一致；**agent-chat 已关闭**，不再自动落盘。与消化引擎解耦。
