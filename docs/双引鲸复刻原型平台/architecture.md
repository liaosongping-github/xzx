...
- 当前 `现网页面复刻原型/` 在文件系统中只含实施追踪，尚无纳入 Git 的可运行复刻应用；浏览器中遗留的本地原型标签不得当作当前分支的源代码证据。
...
新建一个独立的 React + TypeScript + Vite 应用，作为 `现网页面复刻原型/` 的唯一运行时；保留现有 HTML/CSS/片段作为视觉与交互参考，不将其直接作为第二套页面事实源。
