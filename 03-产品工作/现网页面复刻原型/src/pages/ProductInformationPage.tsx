  ...
  return <article className="product-information-page" data-page-id={schema.pageId} onClick={(event) => { if (event.target === event.currentTarget) { setMoreOpen(false); setRowMoreOpen(null); } }}>
    <section className="renderer-list-page">
      ...
      <div className="renderer-page-content">
        ...
        <div className="renderer-toolbar"><div className="renderer-toolbar-left"><button className="dyj-button" type="button" onClick={() => setFormMode("new")}>+ 新建产品</button><button className="renderer-import" type="button">导入产品</button><button type="button">批量删除</button><button type="button">打印条码</button><button type="button">复制产品</button><button type="button">选品车</button><span className="renderer-menu-trigger"><button type="button" onClick={() => setMoreOpen((current) => !current)}>更多操作 &gt;</button>{moreOpen && <div className="renderer-menu renderer-batch-menu" role="menu" aria-label="更多操作菜单">{moreActions.map((action, index) => <button key={action} type="button" role="menuitem">{action}{index === 0 && <span>›</span>}</button>)}</div>}</span></div><div className="renderer-toolbar-right" aria-label="列表工具"><button type="button">⛶</button><button type="button">⛶</button><button type="button">☷</button><button type="button">↻</button><button type="button">⇩</button><button type="button">▣</button><button type="button" className="renderer-icon-button" aria-label="列设置" title="列设置" onClick={() => { setDraftColumns(visibleColumns); setColumnsOpen(true); }}>☷</button></div></div>
        <div className="renderer-table-wrap"><table><thead><tr>{visibleColumns.flatMap((column) => column === "序号" ? [<th key={column}>{column}</th>, <th key="select"><input type="checkbox" aria-label="选择全部产品" /></th>] : [<th key={column}>{column}</th>])}</tr></thead><tbody>{visible.records.length ? visible.records.map((record) => <tr key={record.id}>{visibleColumns.flatMap((column) => column === "序号" ? [<td key={column}>{String(record[column] ?? "—")}</td>, <td key="select"><input type="checkbox" aria-label="选择产品" /></td>] : [<td key={column} className={column === "操作" ? "renderer-actions" : ""}>{column === "操作" ? <><button type="button" onClick={() => setFormMode("edit")}>编辑</button><button type="button" onClick={() => setDeleteOpen(true)}>删除</button><span className="renderer-menu-trigger"><button type="button" onClick={() => setRowMoreOpen((current) => current === record.id ? null : record.id)}>更多</button>{rowMoreOpen === record.id && <div className="renderer-menu renderer-row-menu" role="menu" aria-label="行更多操作菜单">{rowMoreActions.map((action) => <button key={action} type="button" role="menuitem">{action}</button>)}</div>}</span></> : column === "产品图片" ? <span className="renderer-image">{record[column] || ""}</span> : String(record[column] ?? "—")}</td>])}</tr>) : <tr><td colSpan={Math.max(1, visibleColumns.length + 1)}>暂无数据</td></tr>}</tbody></table></div>
